#pragma once
#ifndef WRWATERFALLDATAPALETTE
#define WRWATERFALLDATAPALETTE
...
//表示瀑布图数据调色板。
class WRCHARTING_CLASS WRWaterfallDataPalette
{
...
private:
    ...
    //清理当前对象所占用的相关资源。
    ~WRWaterfallDataPalette();

	//禁用复制构造函数。
	WRWaterfallDataPalette(const WRWaterfallDataPalette& other);
	//禁用赋值操作符。
	WRWaterfallDataPalette& operator=(const WRWaterfallDataPalette& other);

public:
    //初始化色彩。
    inline WRWaterfallDataPalette(const WRColor* list, int size)
        : m_refCnt(1)
    {
        //基本信息
        m_size = size;
...
public:
    //获取默认调色板。
    static WRWaterfallDataPalette* defaultPalette(void);
};

//#pragma warning(pop)

#endif //WRWATERFALLDATAPALETTE
