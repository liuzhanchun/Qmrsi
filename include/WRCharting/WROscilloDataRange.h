...
public:
	//初始化波形图数据范围。
	inline WROscilloDataRange(void)
	{
		//add code here
	}
	//初始化波形图数据范围。
    inline WROscilloDataRange(const WRChartSampleRange& pointRange, const WRChartDValueRange& valueRange)
        : m_pointRange(pointRange), m_valueRange(valueRange)
    {
        //add code here
    }
    //初始化波形图数据范围。
    inline WROscilloDataRange(int pointBegin, int pointCount, double valueBegin, double valueEnd)
        : m_pointRange(pointBegin, pointCount), m_valueRange(valueBegin, valueEnd)
    {
        //add code here
    }
    //初始化波形图数据范围
    inline WROscilloDataRange(const WROscilloDataRange& other)
    ...
    //重载赋点操作符
    inline WROscilloDataRange& operator=(const WROscilloDataRange& other)
    {
        if (this != &other)
        {
